...
export default function ReturnableAuxiliaryTable({
  filtered,
  setRows,
  canUndo,
  undoRows,
  ...
  const totalAmountSum = filtered.reduce((sum, r) => sum + (calcTotalQtyKit(r, auxCols) * (parseFloat(r.returnablePrice) || 0)), 0);
  const finalAmountSum = filtered.reduce((sum, r) => {
    const totalAmount = calcTotalQtyKit(r, auxCols) * (parseFloat(r.returnablePrice) || 0);
    const cycle = parseFloat(r.returnableCycle) || 1;
    return sum + (totalAmount / cycle);
  }, 0);
  ...
  return (
    <>
      ...
      <div style={{ padding: "12px 20px", background: "#f8fafc", borderBottom: "1px solid #dce8f4", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 800, fontSize: 16, color: "#1e3a5f" }}>BOM: Core-Kit_Auxiliary (Returnable)</div>
        ...
        <div style={{ background: "#e0f2fe", border: "1px solid #0284c7", color: "#0284c7", padding: "4px 10px", borderRadius: 4, fontSize: 9, fontWeight: 700, letterSpacing: 1 }}>
          RETURNABLE
        </div>
      </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11, background: "#fff" }}>
          <thead>
            <tr style={{ background: "#dbe8f8", borderBottom: "1px solid #b8cfe8" }}>
              ...
              <th colSpan={6} style={{ ...TH, background: "#fff4e0", color: "#b8600a", fontSize: 8, fontWeight: 800, letterSpacing: 1.2, borderRight: "1px solid #f0d0a0", borderBottom: "2px solid #e6920a88" }}>COST SUMMARY</th>
            ...
            <tr style={{ background: "#f0f5ff", borderBottom: "2px solid #e6920a" }}>
              ...
              <th style={{ ...TH, color: "#b8600a", background: "#fff4e0", textAlign: "right" }}>RET PRICE</th>
              <th style={{ ...TH, color: "#b8600a", background: "#fff4e0", textAlign: "right" }}>TOT AMOUNT</th>
              <th style={{ ...TH, color: "#b8600a", background: "#fff4e0", textAlign: "right" }}>FINAL AMT</th>
              <th style={{ ...TH, color: "#b8600a", background: "#fff4e0", borderRight: "1px solid #e8d5b0", textAlign: "center" }}>RET CYCLE</th>
              <th style={{ ...TH, background: "#f4f6fa", borderRight: "none" }}></th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((r, idx) => {
              ...
              const totalAmount = totalQtyKit * (parseFloat(r.returnablePrice) || 0);
              const cycle = parseFloat(r.returnableCycle) || 1;
              const finalAmount = totalAmount / cycle;
              ...
              return (
                <tr key={r.id} className="row-tr fi" style={{ background: bg, borderBottom: `1px solid #e6920a22` }}>
                  ...
                  {renderEditableCell("returnablePrice", "number", "right")}
                  <td style={{ ...TD, textAlign: "right", fontWeight: 700, color: "#1e3a5f" }}>{fmtCurrency(totalAmount)}</td>
                  <td style={{ ...TD, textAlign: "right", fontWeight: 700, color: "#b8600a" }}>{fmtCurrency(finalAmount)}</td>
                  {renderEditableCell("returnableCycle", "number", "center")}
          ...
          <tfoot>
            <tr style={{ background: "#f0f5ff", borderTop: "2px solid #e6920a" }}>
              ...
              <td style={{ ...TD, background: "#fff4e0" }} />
              <td style={{ ...TD, color: "#1e3a5f", fontWeight: 800, background: "#fff4e0", textAlign: "right" }}>{fmtCurrency(totalAmountSum)}</td>
              <td style={{ ...TD, color: "#b8600a", fontWeight: 800, background: "#fff4e0", textAlign: "right" }}>{fmtCurrency(finalAmountSum)}</td>
              <td style={{ ...TD, background: "#fff4e0", borderRight: "1px solid #e8d5b0" }} />
      ...
      <div style={{ padding: "14px 20px 16px", background: "linear-gradient(180deg,#f8fbff,#eef4fc)", borderTop: "3px solid #e6920a", display: "flex", gap: 10, flexWrap: "wrap", boxShadow: "0 -2px 8px rgba(30,58,95,.06)" }}>
        ...
        <div style={{ marginLeft: "auto", background: "linear-gradient(135deg,#1e3a5f,#2d5a8e)", borderRadius: 8, padding: "10px 20px", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "flex-end", minWidth: 200, boxShadow: "0 2px 8px rgba(30,58,95,.18)" }}>
          <div style={{ fontSize: 8, color: "rgba(255,255,255,.6)", letterSpacing: 2, fontWeight: 700, marginBottom: 4 }}>GRAND TOTAL FINAL AMOUNT</div>
          <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontSize: 22, fontWeight: 800, color: "#e6920a", lineHeight: 1 }}>{fmtCurrency(finalAmountSum)}</div>
        </div>
      </div>
    </>
  );
